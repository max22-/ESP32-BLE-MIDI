...
void onAfterTouchPoly(uint16_t timestamp, uint8_t channel, uint8_t note, uint8_t pressure)
{
...
void onProgramChange(uint16_t timestamp, uint8_t channel, uint8_t program)
{
    Serial.printf("Program change : channel %d, program %d (timestamp %dms)\n", channel, program, timestamp);
}

void onAfterTouch(uint16_t timestamp, uint8_t channel, uint8_t pressure)
{
    Serial.printf("After touch : channel %d, pressure %d (timestamp %dms)\n", channel, pressure, timestamp);
}

void onPitchbend(uint16_t timestamp, uint8_t channel, uint16_t value)
{
